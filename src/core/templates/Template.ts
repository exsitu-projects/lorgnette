...
import { ProgrammableBackwardMapping } from "../mappings/ProgrammableBackwardMapping";
import { ProgrammableForwardMapping } from "../mappings/ProgrammableForwardMapping";
import { TemplateSlot, TemplateSlotKey } from "./TemplateSlot";
import { EvaluatorValue } from "./evaluators/Evaluator";


export const DELETE_SLOT = Symbol("Delete template slot");
export type DeleteSlotSymbol = typeof DELETE_SLOT;

export type TemplateDataValue = EvaluatorValue | DeleteSlotSymbol;
export type TemplateData = Record<TemplateSlotKey, TemplateDataValue>;

export interface TemplateSettings {
    // Transformer functions that can be used to transform the output data (generated by the forward mapping)
    // and the output of the user interface (given to the backward mapping).
...
> {
    ...
    protected getForwardMapping(): ProgrammableForwardMapping<F> {
        return new ProgrammableForwardMapping<F>(({ fragment, document }) => {
    ...
    protected getBackwardMapping(): ProgrammableBackwardMapping<F> {
        return new ProgrammableBackwardMapping<F>(({ fragment, document, documentEditor, userInterfaceOutput }) => {
            const keysToSlots = this.fragmentsToKeysToSlots.get(fragment);
            if (!keysToSlots) {
                throw new Error("The slots could not be retrieved: there is no matching fragment.");
            }

            const transformedOutput = this.settings.transformUserInterfaceOutput(userInterfaceOutput);
    ...
    // in a projection provider using the spread operator (...).
    get resourcesAndMappings() {
        return {
            pattern: this.getFragmentProvider(),
            forwardMapping: this.getForwardMapping(),
            backwardMapping: this.getBackwardMapping(),
        };
    }
}